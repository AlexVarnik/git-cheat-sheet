...
## 17. Слияние и удаление веток

__`git merge <название_ветки>`__ - Выполнить слияние.
...
__`git branch -D <название_ветки>`__ - Удалить ветку после объединения.
...
__Как разрешать конфликты: общие рекомендации__
...
__`git push`__ - Отправить локальную ветку в удалённый репозиторий.
...
- `Create a pull request for 'feature/merge-request' on GitHub by visiting`: — предоставляет ссылку, чтобы быстро создать запрос на изменения. Подробнее о таких запросах расскажем в следующем разделе.
